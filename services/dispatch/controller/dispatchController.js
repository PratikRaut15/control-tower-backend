...
createDispatch = async function(req, res, next) {
  ...
  if (req.body.route) {
    route = await getRouteIdByName(req.body.route);
  ...
  if (req.body.vehicleType) {
    vehicleType = await getVehicleTypeByName(req.body.vehicleType);
    if (vehicleType === null) {
      return res.json({
        code: 204,
...
getTransporterName = function(req, res, next) {
  return new Promise(async (resolve, reject) => {
    try {
      const data = await dispatchModel.getTransporterName(req);
      if (data) {
        resolve(data);
      } else {
        reject(false);
...
getDispatchDetails = vehicleNo => {
  return new Promise(async (resolve, reject) => {
    ...
    const arrDetails = await dispatchModel.getDispatchDetails(vehicleNo);

    arrResult["vehicleNo"] = vehicleNo;
    if (arrDetails.driverMasterId) {
      arrResult["driverName"] = await dispatchModel.getDriverNameById(
        arrDetails.driverMasterId
      );
    }

    if (arrDetails.driverMasterId) {
      arrResult["DriverNumber"] = await dispatchModel.getDriverNumberById(
        arrDetails.driverMasterId
      );
    }

    if (arrDetails.shipmentDeliveryMappingId) {
      arrResult["ShipmentNumber"] = await dispatchModel.getShipmentNumber(
        arrDetails.shipmentDeliveryMappingId
      );
    }
    if (arrDetails.transporterMasterId) {
      arrResult["transporterNumber"] = await dispatchModel.getTransporterName(
        arrDetails.transporterMasterId
      );
    }
    ...
    if (arrDetails.transporterMasterId) {
      arrResult[
        "transporterLRNumber"
      ] = await dispatchModel.getTransporterLRNumber(
        arrDetails.transporterMasterId
      );
    }

    if (arrDetails.vehicleTypeMasterId) {
      arrResult["vehicleType"] = await dispatchModel.getvehicleTypeMaster(
        arrDetails.vehicleTypeMasterId
      );
    }
