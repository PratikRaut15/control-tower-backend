...
getSourceNameById = function(req, tripId) {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT checkPointName FROM checkpointmaster WHERE checkPointMasterId=?`,
      [req],
      function(error, results, fields) {
        try {
          if (error) {
            ...
            resolve(results[0]["checkPointName"]);
          }
        } catch (error) {
          reject(500);
        }
      ...
      function(error, results, fields) {
        try {
          if (error) {
            ...
            resolve(results[0]["transporterName"]);
            /* if(results.length>0){
                      res(null,1);
                  } else {
                      res(null,0);
...
getDispatchDetails = vehicleNo => {
  return new Promise(async (resolve, reject) => {
    const response = await getTripDetails(vehicleNo);
    if (response) {
      resolve(response);
    } else {
      reject(false);
...
getTripDetails = vehicleNo => {
  return new Promise((resolve, reject) => {
    let sql = `SELECT tripId,sourceId,destinationId,vehicleNo,transporterMasterId,tripStatusMasterId,vehicleTypeMasterId,driverMasterId,shipmentDeliveryMappingId from trip where vehicleNo = '${vehicleNo}'`;
    console.log(sql);
    dbConnection.pool.query(sql, (err, results) => {
      if (err) {
        reject(err);
      } else {
        resolve(results[0]);
...
getShipmentNumber = shipmentDeliveryMappingId => {
  return new Promise((resolve, reject) => {
    let sql = `SELECT shipmentId from shipmentdeliverymapping where shipmentDeliveryMappingId = '${shipmentDeliveryMappingId}'`;
    try {
      dbConnection.pool.query(sql, (err, results) => {
        console.log("err", err);
        ...
        if (err) {
          ...
          if (results.length > 0) {
            if (results[0]["shipmentId"]) {
              let sql2 = `SELECT shipmentNo from shipment where shipmentId = '${results[0]["shipmentId"]}'`;
              dbConnection.pool.query(sql2, (err, results) => {
                resolve(results[0]["shipmentNo"]);
              });
            } else {
              resolve("NA");
            }
...
getDriverNameById = function(req, res) {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT DriverName FROM drivermaster WHERE driverMasterId=?`,
      [req],
...
getDriverNumberById = function(req, res) {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT DriverMobileNo FROM drivermaster WHERE driverMasterId=?`,
      [req],
      function(error, results, fields) {
        try {
          if (error) {
            ...
            resolve(results[0]["DriverMobileNo"]);
          }
        } catch (error) {
          reject(500);
        }
...
getTransporterLRNumber = function(req, res) {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT transporterCode FROM transportermaster WHERE transporterMasterId=?`,
      [req],
      function(error, results, fields) {
        try {
          if (error) {
            ...
            resolve(results[0]["transporterCode"]);
          }
        } catch (error) {
          reject(500);
        }
...
getvehicleTypeMaster = function(req, res) {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT type FROM vehicletypemaster WHERE vehicleTypeMasterId=?`,
      [req],
...
getVehicleNo = (req, res) => {
  return new Promise((resolve, reject) => {
    dbConnection.pool.query(
      `SELECT vehicleNo FROM trip WHERE vehicleNo=?`,
      [req],
      function(error, results, fields) {
        try {
          if (error) {
